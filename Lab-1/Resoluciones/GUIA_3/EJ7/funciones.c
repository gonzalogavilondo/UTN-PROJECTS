...
void insertarEnOrden(Pila *pila, int dato)
{
    ...
    ///Vuelvo a apilar todo lo mayor al dato a la pila, quedando el dato en el "medio"
    while (!pilavacia(&aux))
    {
...
void ordenamientoPorInsercion(Pila *origen, Pila *destino)
{
    while(!pilavacia(origen))
    {
        insertarEnOrden(destino, desapilar(origen)); ///Vamos a ir apilando en destino lo que desapilamos de origen pero insertando en orden.
    }
}
