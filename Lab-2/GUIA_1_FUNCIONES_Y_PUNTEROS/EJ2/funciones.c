...
/**
    InvertirVectorString
    Parametros: Vector y cantidad de elementos validos
    Invierte el vector de caracteres.
**/
char *InvertirVectorString(char *vector, int validos)
{
    char *vectorStringInvertido = NULL;
    int j = 0;

    ///Reservo memoria con la cantidad de elementos pasados por parametro, ya que es el mismo vector invertido.
    vectorStringInvertido = ReservarMemoriaString(validos);

    for(int i = validos-1; i >= 0; i--) //Validos incluye al '\0' al contar.
    {
        vectorStringInvertido[j] = vector[i];
        j++;
    }

    return vectorStringInvertido;
}
